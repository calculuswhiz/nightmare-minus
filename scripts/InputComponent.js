...
class DisplayFilePath extends React.Component {
	render() {
		return React.createElement(
			'div',
			{ className: 'DisplayFilePath', id: this.props.id },
			'Current ROM: ',
			this.props.path
		);
	}
...
class ModuleSelector extends React.Component {
	...
	render() {
		...
		return React.createElement(
			'div',
			{ className: 'ModuleSelector', id: this.props.id },
			"Current Module: ",
			this.props.curModIndex == -1 ? "No modules loaded" : React.createElement(
				'select',
				{ id: 'selComponent', value: this.props.curModIndex, onChange: this.selectModule },
...
class NEDLink extends React.Component {
	...
	render() {
		return React.createElement(
			...
			React.createElement(
				'a',
				{ id: `converted${this.props.index}`, onClick: this.clickHandler, href: '#' },
				`${this.state.value}`
			)
		);
	}
	...
	render() {
		...
		switch (this.props.type) {
			case "TEXT":
				...
				var inputElements = React.createElement('input', { type: 'text', maxLength: this.props.EntrySize,
					className: 'TEXT', value: valueString });
				break;
			case "HEXA":
				// @todo:implement onchange action
				// 2 characters per byte:
				var inputElements = React.createElement('input', { type: 'text', maxLength: 2 * this.props.EntrySize,
			...
			case "NEDU":
				// Precede with 0x 
				var inputElements = React.createElement(NEDLink, { type: this.props.type.slice(2), index: this.props.index, value: `0x${valueString}`, EntrySize: this.props.EntrySize,
					changeHandler: this.changeValue });
				break;
			// Dropdown menus:
			case "NDHU":
				var inputElements = React.createElement(
					'select',
					{ id: `DH${this.props.index}`, value: parseInt(`0x${valueString}`), onChange: this.changeDrop },
					this.props.DropdownParams.map((param, idx) => {
						return React.createElement(
							'option',
							{ key: idx, value: parseInt(param.value) },
			...
			case "NDDU":
				var inputElements = React.createElement(
					'select',
					{ id: `DD${this.props.index}`, value: parseInt(`0x${valueString}`), onChange: this.changeDrop },
					this.props.DropdownParams.map((param, idx) => {
						return React.createElement(
							'option',
							{ key: idx, value: parseInt(param.value) },
		...
		return React.createElement(
			...
			EntryComponents.map((entry, idx) => {
				...
				return React.createElement(
					...
					React.createElement(
						'div',
						{ className: 'entryLabel', title: `0x${address.toString(16)}` },
						entry.EntryInfo
					),
					React.createElement(FieldInput, { type: entry.EditElement, location: address, DropdownParams: entry.DropdownParams, EntrySize: entry.EntrySize, index: idx, writeROMBufferHandler: this.props.writeROMBufferHandler, readROMBufferHandler: this.props.readROMBufferHandler })
				);
			})
		);
	}
	...
	render() {
		...
		return React.createElement(
			...
			React.createElement(
				...
				React.createElement(
					'select',
					{ id: 'selEntry', value: this.state.entryIndex, onChange: this.selectEntry },
				...
				React.createElement(TableEntryEditor, { curModule: curModule, index: this.state.entryIndex, writeROMBufferHandler: this.props.writeROMBufferHandler, readROMBufferHandler: this.props.readROMBufferHandler })
			)
		);
	}
}
	...
	render() {
		...
		return React.createElement(
			...
			React.createElement(
				...
				React.createElement(
					'a',
					{ href: '#', onClick: this.selectROM, className: 'linkButton', onDragStart: e => {
							e.preventDefault();
						} },
					'Open ROM...'
				),
				...
				React.createElement(
					'a',
					{ href: '#', onClick: this.saveROMAs, className: 'linkButton', onDragStart: e => {
							e.preventDefault();
						} },
					'Save ROM As...'
				)
			),
			React.createElement(DisplayFilePath, { id: 'ROMFileDisp', path: this.state.ROMFilePath }),
			React.createElement(
				'div',
				{ id: 'ButtonContainer' },
				React.createElement(
					'a',
					{ href: '#', onClick: this.openModule, className: 'linkButton', onDragStart: e => {
							e.preventDefault();
						} },
					'Open Module...'
				),
				...
				React.createElement(
					'a',
					{ href: '#', onClick: this.closeModule, className: 'linkButton', onDragStart: e => {
							e.preventDefault();
						} },
					'Close Active Module'
				),
				...
				React.createElement(
					'a',
					{ href: '#', onClick: this.convertModuleToJSON, className: 'linkButton', onDragStart: e => {
							e.preventDefault();
						} },
					'Convert NMM to JSON'
				)
			),
			React.createElement(ModuleSelector, { id: 'ModFileDisp', moduleData: this.state.ModData, curModIndex: moduleIndex, changeModuleIndex: this.changeModuleIndex }),
			this.state.curModIndex != -1 ? React.createElement(EditorMain, { id: 'EditorMain', curModule: moduleObject, modIndex: moduleIndex, writeROMBufferHandler: this.writeROMBuffer, readROMBufferHandler: this.readROMBuffer }) : null
		);
	}
}
